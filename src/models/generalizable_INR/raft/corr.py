...
try:
    from .triton_corr import triton_corr_forward
except:
    pass
...
class AlternateCorrBlock:
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            r = self.radius
            fmap1_i = self.pyramid[0][0] # Raw [B, C, H, W]
            fmap2_i = self.pyramid[i][1] # Raw [B, C, H, W]

            coords_i = coords / (2**i)   # Raw [B, 2, H, W]

            # Your triton kernel natively understands NCHW strides
            # Returns clean [B, (2*r+1)^2, H, W] tensor
            corr = triton_corr_forward(fmap1_i, fmap2_i, coords_i, r)
